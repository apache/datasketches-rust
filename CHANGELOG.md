...
* REQ deserialization now rejects compactors whose section count is not on the REQ doubling schedule, whose rounded section size is below `MIN_K`, whose `lg_weight` does not match the enclosing level, or whose retained weighted count disagrees with `n`.
