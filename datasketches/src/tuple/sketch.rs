...
mod private {
    ...
    // Sealed trait to prevent external implementations of TupleSketchView.
    pub trait Sealed {}

    impl<S, P> Sealed for UpdatableTupleSketch<S, P> {}
    impl<S> Sealed for CompactTupleSketch<S> {}
}

/// Read-only view for Tuple sketches.
///
/// This trait provides a unified input abstraction for APIs (such as union and intersection) that
/// can accept either a mutable [`UpdatableTupleSketch`] or an immutable [`CompactTupleSketch`].
/// `S` is the summary type retained by the sketch.
pub trait TupleSketchView<S>: private::Sealed {
    ...
    fn iter<'a>(&'a self) -> impl Iterator<Item = (u64, &'a S)> + 'a
    where
        S: 'a;
    ...
    /// Returns whether retained entries are ordered in ascending order by hash.
...
impl<S: Clone, P> UpdatableTupleSketch<S, P> {
    ...
    pub fn compact(&self, ordered: bool) -> CompactTupleSketch<S> {
        ...
        // Match Theta's behavior for never-updated sketches initialized with p < 1.0.
        let theta = if empty { MAX_THETA } else { self.table.theta() };
        let is_single = entries.len() == 1 && theta == MAX_THETA;
        // Empty or single-item sketches are always ordered (Java/C++ compatibility).
        ...
        CompactTupleSketch::from_parts(entries, theta, self.table.seed_hash(), ordered, empty)
    }
}

impl<S, P> TupleSketchView<S> for UpdatableTupleSketch<S, P> {
    fn seed_hash(&self) -> u16 {
        UpdatableTupleSketch::seed_hash(self)
    }

    fn theta64(&self) -> u64 {
        UpdatableTupleSketch::theta64(self)
    }

    fn is_empty(&self) -> bool {
        UpdatableTupleSketch::is_empty(self)
    }
    ...
    where
        S: 'a,
    {
        UpdatableTupleSketch::iter(self)
    }

    fn num_retained(&self) -> usize {
        UpdatableTupleSketch::num_retained(self)
    }
}
...
impl<S> CompactTupleSketch<S> {
        ...
        SD: SummarySerde<S>,
    {
        ...
        let mut flags = serialization::FLAGS_IS_READ_ONLY | serialization::FLAGS_IS_COMPACT;
        if self.is_empty() {
...
impl<S> TupleSketchView<S> for CompactTupleSketch<S> {
    fn seed_hash(&self) -> u16 {
        CompactTupleSketch::seed_hash(self)
    }

    fn theta64(&self) -> u64 {
        CompactTupleSketch::theta64(self)
    }
    ...
    where
        S: 'a,
    {
        CompactTupleSketch::iter(self)
    }

    fn num_retained(&self) -> usize {
        CompactTupleSketch::num_retained(self)
    }

    fn is_ordered(&self) -> bool {
        CompactTupleSketch::is_ordered(self)
    }
}
...
mod tests {
    ...
    fn view_summary_sum<V: TupleSketchView<u64>>(view: &V) -> u64 {
        view.iter().map(|(_, &summary)| summary).sum()
    }
