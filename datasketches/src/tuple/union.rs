...
//! Tuple sketch union.
//!
//! [`TupleUnion`] computes the union (set OR) of any number of Tuple sketches. Like the Theta union
//! it keeps an internal "gadget" hash table plus a running `union_theta`, and updates it from each
//! input sketch. The only Tuple-specific behavior is that when an incoming key already exists in
//! the gadget, the two summaries are combined with a [`SummaryCombinePolicy`] instead of one being
//! dropped.
...
impl<S, P> TupleUnion<S, P> {
        ...
        S: Clone,
    {
        ...
        // Any non-empty input makes the union non-empty, even if every key is screened out by
        // theta.
        self.table.set_empty(false);
        self.union_theta = self.union_theta.min(sketch.theta64());

        let ordered = sketch.is_ordered();
        let policy = &self.policy;
        for (hash, summary) in sketch.iter() {
            // A key contributes only if it is below both the running union theta and the gadget
            // theta (the gadget theta can drop while we insert). For an ordered input,
            // the first key that fails this test means all remaining (larger) keys fail
            // too, so we can stop early.
            ...
            self.table.upsert(hash, |existing| match existing {
                Some(existing_summary) => {
                    policy.combine(existing_summary, summary);
                    None
                }
        ...
        // A rebuild during the inserts above may have lowered the gadget theta below union_theta.
        ...
        S: Clone,
    {
        ...
        if self.table.is_empty() {
            return CompactTupleSketch::from_parts(
                Vec::new(),
                self.union_theta,
                seed_hash,
                true,
        ...
        // Trim down to the nominal size k, lowering theta to the k-th smallest hash if needed.
        let nominal = 1usize << self.table.lg_nom_size();
        if entries.len() > nominal {
            let (_, kth, _) = entries.select_nth_unstable_by_key(nominal, |(hash, _)| *hash);
            theta = kth.0;
            entries.truncate(nominal);
        }
        ...
        CompactTupleSketch::from_parts(entries, theta, seed_hash, ordered, false)
    }
...
mod tests {
    ...
    fn union_result_is_order_independent() {
        ...
        assert_eq!(
            sorted_entries(&a_then_b.result(true)),
            sorted_entries(&b_then_a.result(true))
        );
    }
