...
fn bounds_are_nested_and_in_unit_interval() {
    let mut sketch =
        ReqSketch::try_new(12, RankAccuracy::HighRank).expect("construction should succeed");
...
fn hra_and_lra_bounds_are_tighter_at_their_target_end() -> Result<(), Error> {
    for rank in [0.05, 0.25, 0.5, 0.75, 0.95] {
        let mut hra = ReqSketch::try_new(12, RankAccuracy::HighRank)?;
        let mut lra = ReqSketch::try_new(12, RankAccuracy::LowRank)?;
